    ...
    while len(words_letters) > 0 and lives > 0:            #need to loop as until word is guessed fully
        ...
        if user_letter in alphabet - letters_used:          #Taking used letters out of alphabet string and seeing if user's guess is in the alphabet                
            ...
            if user_letter in words_letters:                #If statement to check if user's letter is in the word's letters 
                ...
                print('Sorry, your letter was not in the word. Try one more time (maybe)!') #Keeping frustration to a minimum 
                time.sleep(.5)                              #Delay
...
def PlayAgain():
    ...
    if lives == 0:                                          #If lives are out tell user that they have lost
        time.sleep(1)                                       #Dlay 
        print('Sorry, you have no lives. Sucks to suck.')
        Keep_going ()                                       #See if the player wants to play again
    else:                                                   #If the user still has lives and a while loop's parameter has been met the user has Won | Tell them    
        time.sleep(1)                                       #Delay
        print('The word was',word,'.Great job!')
        Keep_going()                                        #See if player wants to play again 

def Keep_going():                                                   #Function to repeate the game if user wants. Differs from Hangman game because there is no introduction
    x = 'N'                                                         #Variable to initiate start of while loop
    while x != 'Y':                                                 #While loop to keep the player playing
            x = input('Do you want to keep going(Y/N)? ').upper()   #Asking User if they want play again changing letters to uppercse
            if x == 'Y':                                            #If player wants to play again play again
                PlayAgain()                                         #Calling The playagain hangman function 
            elif x != 'Y' and x!= 'N':                              #If its not Y or N tell player to enter acceptable value
                print('Invalid letter. Please put Y or N') 
            else:                                                   #Else Statement for a No
                print('Thanks for playing!')
                return x                                            #Returning the N value of x stops the code entirely
